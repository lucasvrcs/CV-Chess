...
for filename in sorted(os.listdir(image_path)):
    if filename.endswith(".jpg"): 
        ...
        if img is not None:
            images.append((img, filename))  # Store the filename with the image
...
for idx, (image, filename) in enumerate(images):
    ...
    # Save the RGB image instead of showing it
    plt.figure(figsize=(9, 7))
    plt.imshow(rgb_image)
    plt.title(f"RGB Image {idx+1}: {filename}")
    plt.axis('off')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"{idx+1}_{os.path.splitext(filename)[0]}_rgb.png"))
    plt.close()  # Close the figure instead of showing it
    
    ##########################################################

    blurred = cv2.GaussianBlur(gray_image, (3, 3), 1.5)
    ...
    # Canny edge detection (no plot as requested)
    canny_image = cv2.Canny(otsu_binary, 30, 255)

    # Dilation to connect nearby edges
    kernel = np.ones((9, 9), np.uint8)
    dilation_image = cv2.dilate(canny_image, kernel, iterations=1)

    # Save dilation image instead of showing it
    plt.figure(figsize=(9, 7))
    plt.title(f"Grid Lines Overlay: {filename}")
    plt.imshow(dilation_image)
    plt.savefig(os.path.join(output_dir, f"{idx+1}_{os.path.splitext(filename)[0]}_dilation.png"))
    plt.close()

    # Hough Lines with adjusted parameters
    lines = cv2.HoughLinesP(dilation_image, 1, np.pi / 180, threshold=550, minLineLength=225, maxLineGap=110)
    ...
    if lines is not None:
        ...
        # Save filtered grid lines instead of showing them
        plt.figure(figsize=(9, 7))
        plt.title(f"Filtered Grid Lines: {filename}")
        plt.imshow(black_image, cmap="gray")
        plt.savefig(os.path.join(output_dir, f"{idx+1}_{os.path.splitext(filename)[0]}_filtered_grid.png"))
        plt.close()
        ...
        # Save the overlay instead of showing it
        plt.figure(figsize=(9, 7))
        plt.title(f"Grid Lines Overlay: {filename}")
        plt.imshow(overlay_image)
        plt.savefig(os.path.join(output_dir, f"{idx+1}_{os.path.splitext(filename)[0]}_grid_overlay.png"))
        plt.close()
        ...
        # Save results instead of showing them
        plt.figure(figsize=(18, 15))
        plt.subplot(131)
        plt.title(f"Geometrically valid squares on original image: {filename}")
        plt.imshow(cv2.cvtColor(squares_image, cv2.COLOR_BGR2RGB))
        plt.subplot(132)
        plt.title(f"Geometrically valid squares: {filename}")
        plt.imshow(valid_squares_image, cmap="gray")
        plt.subplot(133)
        plt.title(f"All detected quadrilaterals: {filename}")
        plt.imshow(all_contours_image, cmap="gray")
        plt.savefig(os.path.join(output_dir, f"{idx+1}_{os.path.splitext(filename)[0]}_valid_squares.png"))
        plt.close()
        ...
        # Save dilated image instead of showing it
        plt.figure(figsize=(12,8))
        plt.title(f"dilated_valid_squares_image: {filename}")
        plt.imshow(dilated_valid_squares_image,cmap="gray")
        plt.savefig(os.path.join(output_dir, f"{idx+1}_{os.path.splitext(filename)[0]}_dilated.png"))
        plt.close()
        ...
        if contours:
            ...
            # Save the convex hull mask instead of showing it
            plt.figure(figsize=(12, 8))
            plt.title(f"Convex Hull Mask: {filename}")
            plt.imshow(hull_mask, cmap="gray")
            plt.savefig(os.path.join(output_dir, f"{idx+1}_{os.path.splitext(filename)[0]}_hull_mask.png"))
            plt.close()
            ...
            plt.figure(figsize=(12, 8))
            plt.title(f"Filled Chessboard (Convex Hull): {filename}")
            plt.imshow(filled_chessboard, cmap="gray")
            plt.savefig(os.path.join(output_dir, f"{idx+1}_{os.path.splitext(filename)[0]}_filled_chessboard.png"))
            plt.close()
        ...
        if not contours:
            ...
            plt.figure(figsize=(12, 8))
            plt.title(f"Corner Detection (Ordered by Detection): {filename}")
